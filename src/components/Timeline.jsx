...
const Timeline = ({ onNext }) => {
    ...
    return (
        <div className="py-20 px-4 w-full max-w-4xl fade-in">
            <h2 className="text-4xl md:text-6xl font-cinzel text-center text-gold-gradient mb-20 tracking-widest">Our Timeline</h2>

            <div className="relative border-l-2 border-gold-600/30 ml-4 md:ml-auto md:mr-auto">
                {events.map((event, index) => (
                    <div key={index} className="mb-16 ml-8 relative">
                        {/* Timeline Dot */}
                        <div className="absolute -left-[41px] top-1 w-5 h-5 bg-gold-500 rounded-full shadow-[0_0_15px_rgba(212,175,55,0.8)] border-4 border-black"></div>

                        <div className="bg-black/40 p-6 md:p-8 rounded-xl border border-gold-600/20 shadow-xl backdrop-blur-sm">
            ...
            <div className="flex justify-center mt-12 w-full">
                <button
                    onClick={onNext}
                    className="btn-neon px-12 py-4 rounded-full font-cinzel font-bold uppercase tracking-[0.2em] text-xs"
                >
                    Look Back Further
                </button>
            </div>
